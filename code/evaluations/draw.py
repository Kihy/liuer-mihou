...
font_path = '/usr/share/fonts/truetype/cmu/cmunrm.ttf'  # the location of the font file
my_font = fm.FontProperties(fname=font_path)

def plot_comparison(file_list, ml_list):
    num_files = len(file_list)
    num_models = len(ml_list)
    fig, ax = plt.subplots(num_files, num_models, sharey='row', figsize=(12,8))


    for i, filename in enumerate(file_list):
        attack_file=f"../ku_dataset/{filename}"
        ...
        for j, ml_name in enumerate(ml_list):
            ml_score = np.genfromtxt(attack_file + f"_{ml_name.lower()}_score.csv")
            ml_threshold = np.genfromtxt(attack_file + f"_{ml_name.lower()}_threshold.csv")
            # print(ml_threshold)
            ...
            ax[i, j].scatter(range(len(imposter_score)), imposter_score, s=0.1, alpha=0.5,c="#DA7C25",label="Surrogate")
            ax[i, j].axhline(y=threshold, color="#DA7C25")

            ax[i, j].scatter(range(len(ml_score)), ml_score, s=0.1, alpha=0.5,c="#2583DA",label=f"{ml_name}")
            ax[i, j].legend(loc="lower right", markerscale=10)
            ax[i, j].axhline(y=ml_threshold, color="#2583DA")
            ax[i, j].set_yscale('log')
            if i==0:
                    ax[i,j].set_title(ml_list[j],fontproperties=my_font, fontsize=18)
            if j==0:
                if i==0:
                    ax[i,j].set_ylabel("PS",fontproperties=my_font, fontsize=18)
                if i==1:
                    ax[i,j].set_ylabel("OD",fontproperties=my_font, fontsize=18)
                if i==2:
                    ax[i,j].set_ylabel("HF",fontproperties=my_font, fontsize=18)


            ax[i, j].ticklabel_format(axis='x', style='sci', scilimits=(0,0), useMathText=True, useOffset=True)
...
def plot_results(file_list):
    ...
    fig, ax = plt.subplots(num_attacks, num_files, sharey='row', figsize=(8,8))
    ...
    for i in range(num_files):
        for j in range(num_attacks):
            ...
            if i==0:
                if j==0:
                    ax[i,j].set_title("Original",fontproperties=my_font, fontsize=18)

                if j==1:
                    ax[i,j].set_title("Adversarial",fontproperties=my_font, fontsize=18)

                if j==2:
                    ax[i,j].set_title("Replayed",fontproperties=my_font, fontsize=18)
            if j==0:
                if i==0:
                    ax[i,j].set_ylabel("PS",fontproperties=my_font, fontsize=18)
                if i==1:
...
if __name__ == '__main__':
    ...
    # plot_results(files)
    plot_comparison(["port_scan_attack_only","[OS & service detection]traffic_GoogleHome_av_only","flooding_attacker_only"],["Kitsune","SOM","LOF","RRCF","OCSVM"])
